...
for sim_setting in [3]:
    ...
    for i_sim in range(n_sim):
        ...
        # estimate E(Y|X,S,A), and get the estimated outcome under each S and treatment
        Ymat=pd.DataFrame()
        for i in range(len(data_norm[A_name].unique())):
            ...
            train_data_dic = utils.df_to_dic(train, 'S', X_names_exp, Y_name)
            utils.save_dic(train_data_dic, train_path)   
            
            test = pd.DataFrame()
            test_new = data_norm.copy()
            test_new['S_true'] = test_new['S']
            test_new['A_true'] = test_new['A']
            test_new[A_name] = i
            ...
            test_data_dic = utils.df_to_dic(test, 'S', X_names_exp, Y_name)
            utils.save_dic(test_data_dic, test_path)
            
            # best by auc
            Ymat['Y'+str(i)] = FedAvg(data='flori', model='reg', num_rounds=500, num_f=len(X_names_exp), 
        ...
        Y0mat = np.array(Ymat['Y0']).reshape(nhosp, data_norm.shape[0]).T
        Y1mat = np.array(Ymat['Y1']).reshape(nhosp, data_norm.shape[0]).T
        Y2mat = np.array(Ymat['Y2']).reshape(nhosp, data_norm.shape[0]).T
        ...
        for i in range(data_norm.shape[0]):
            ...
            Y2_hat_new = Y2mat[i, true_hosp]
            Y0_hat.append(Y0_hat_new)
            Y1_hat.append(Y1_hat_new)
            Y2_hat.append(Y2_hat_new)
                
        data_norm['Y0_hat'] = Y0_hat
        data_norm['Y1_hat'] = Y1_hat
        data_norm['Y2_hat'] = Y2_hat
        data_norm['Yhat'] = np.where((data_norm[A_name]==0), data_norm['Y0_hat'], 
                                     np.where((data_norm[A_name]==1), data_norm['Y1_hat'], 
                                              data_norm['Y2_hat']))
        ...
        Y2mat_test = pd.DataFrame(Y2mat)
        Y2mat_test['data'] = data_norm['data']
        Y2mat_test = Y2mat_test[Y2mat_test['data']=='test']
        Y2mat_test = Y2mat_test.drop(columns = 'data').reset_index(drop=True)
        Y2mat_min = Y2mat_test.min(axis=1)
        ...
        for method in ['flori', 'exp']:
            ...
            for tk in [0,1,2]:
                ...
                for tj in cond_j:
                    ...
                    train_data_norm_sub = train_data_norm[(train_data_norm[A_name]==tk) | (train_data_norm[A_name]==tj)].copy()
                    train_data_norm_sub[A_name] = np.where((train_data_norm_sub[A_name]==tk), 1, 0).copy()
                    train_data_norm_sub = train_data_norm_sub.reset_index(drop=True)
                    
                    test_data_norm_sub = test_data_norm[(test_data_norm[A_name]==tk) | (test_data_norm[A_name]==tj)].copy()
                    test_data_norm_sub[A_name] = np.where((test_data_norm_sub[A_name]==tk), 1, 0).copy()
                    test_data_norm_sub = test_data_norm_sub.reset_index(drop=True)
                    
                    # estimate E(Y|X,S,A), and get the estimated outcome under each hospital and treatment, given trt=k/j
                    Ymat_kj=pd.DataFrame()
                    for i in range(len(train_data_norm_sub[A_name].unique())):
                        ...
                        train = train_data_norm_sub[train_data_norm_sub[A_name]==i].reset_index(drop=True).copy()
                        train_data_dic = utils.df_to_dic(train, 'S', X_names_exp, Y_name)
                        utils.save_dic(train_data_dic, train_path)
                        ...
                        Ymat_kj['Y'+str(i)] = FedAvg(data='flori', model='reg', num_rounds=500, num_f=len(X_names_exp), 
                                                     layers=2, units=1024, drop_rate=0.2, 
                                                     learning_rate=0.01, num_epochs=1, 
                                                     batch_size=32, class_weight=1)
                        
                    Y0mat_kj = np.array(Ymat_kj['Y0']).reshape(nhosp, data_norm.shape[0]).T
                    Y1mat_kj = np.array(Ymat_kj['Y1']).reshape(nhosp, data_norm.shape[0]).T


                    # estimated E(Y|X,S,A) under the actual hospital, given trt=k/j
                    Y0_hat_kj = []
                    Y1_hat_kj = []
                    ...
                    for i in range(data_norm.shape[0]):
                        ...
                        true_hosp = data_norm['S'][i]
                        Y0_hat_new = Y0mat_kj[i, true_hosp]
                        Y1_hat_new = Y1mat_kj[i, true_hosp]
                        Y0_hat_kj.append(Y0_hat_new)
                        Y1_hat_kj.append(Y1_hat_new)
                            
                    data_norm['Y0_hat_kj'] = Y0_hat_kj
                    data_norm['Y1_hat_kj'] = Y1_hat_kj
                    data_norm['Yhat_kj'] = np.where((data_norm[A_name]==1), data_norm['Y1_hat_kj'], data_norm['Y0_hat_kj'])
                    ...
                    if method == 'flori':
                        ...
                        g_qua0 = Y0mat_kj.min(axis = 1)
                        g_qua1 = Y1mat_kj.min(axis = 1)
                        y_qua = g_qua1-g_qua0
                        ...
                        #train_data_dic = utils.df_to_dic(train_data_norm_sub, 'S', X_names, 'label_flori')
                        #utils.save_dic(train_data_dic, train_path)
                        #test_data_dic = utils.df_to_dic(test_data_norm, 'S', X_names, 'label_flori')
                        #utils.save_dic(test_data_dic, test_path)
                        
                        y_pred_df['y_pred_'+str(tk)+str(tj)] = FedAvg('flori', 'reg', num_rounds, num_f=len(X_names),
                                                                      layers=2, units=1024, drop_rate=0.2, 
                                                                      learning_rate=0.01, num_epochs=1, batch_size=32, class_weight=1)
                        #y_pred_df['y_pred_'+str(tk)+str(tj)] = test_data_norm['label_flori']
                    ...
                    if method == 'exp':
                        ...
                        # use this to predict after the model is tuned
                        test_data_dic = utils.df_to_dic(data_norm, 'S', X_names, 'Yhat_kj')
                        utils.save_dic(test_data_dic, test_path)

                        for i in range(len(train_data_norm_sub[A_name].unique())):
                            train = train_data_norm_sub[train_data_norm_sub[A_name]==i].reset_index(drop=True).copy()
                            train_data_dic = utils.df_to_dic(train, 'S', X_names, 'Yhat_kj')
                            utils.save_dic(train_data_dic, train_path)
                            data_norm['Y'+str(i)+'_tild'] = FedAvg('flori', 'reg', num_rounds=100, num_f=len(X_names), 
                                                                   layers=2, units=1024, drop_rate=0.2, 
                                                                   learning_rate=0.0001, num_epochs=10, 
                                                                   batch_size=1, class_weight=1)
                        ...
                        #train_data_dic = utils.df_to_dic(train_data_norm_sub, 'S', X_names, 'label_exp')
                        #utils.save_dic(train_data_dic, train_path)
                        ...
                        y_pred_df['y_pred_'+str(tk)+str(tj)] = FedAvg('flori', 'reg', num_rounds, num_f=len(X_names), 
                                                                      ...
                                                                      learning_rate=0.01, num_epochs=1, 
                                                                      batch_size=32, class_weight=1)
                        #y_pred_df['y_pred_'+str(tk)+str(tj)] = test_data_norm['label_exp']
                    ...
                    if method == 'naive':
                        ...
                        for i in range(len(train_data_norm_sub[A_name].unique())):
                            ...
                            utils.save_dic(train_data_dic, train_path)
                            data_norm['Y'+str(i)+'_tild'] = FedAvg('flori', 'reg', num_rounds, num_f=len(X_names), 
                                                                   layers=2, units=1024, drop_rate=0.2, 
                                                                   learning_rate=0.0001, num_epochs=10, batch_size=1, class_weight=1)
                        ...
                        train_data_dic = utils.df_to_dic(train_data_norm_sub, 'S', X_names, 'label_naive')
                        utils.save_dic(train_data_dic, train_path)
            ...
            # evaluation
            f = pd.DataFrame()
            f['f0'] = y_pred_df['y_pred_01'] + y_pred_df['y_pred_02'] 
            f['f1'] = y_pred_df['y_pred_10'] + y_pred_df['y_pred_12']    
            f['f2'] = y_pred_df['y_pred_21'] + y_pred_df['y_pred_20']     

            A_pred = f.idxmax(axis=1)
            A_pred = np.where((A_pred == "f0"), 0, np.where((A_pred == "f1"), 1, 2))

            test_df['A_pred'] = A_pred.astype(int)
            #np.mean(test_df['A_pred']==test_df['d'])
            
            ## reward
            test_df['Yhat_pred'] = np.where((test_df['A_pred']==0), test_df['Y0_true'], 
                                            np.where((test_df['A_pred']==1), test_df['Y1_true'], test_df['Y2_true']))
